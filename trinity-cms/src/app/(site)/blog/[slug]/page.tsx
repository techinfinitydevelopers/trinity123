      ...
      <article className="post" itemScope itemType="https://schema.org/Article">
        <header className="post__hero">
          ...
          <div className="container post__hero-inner">
            ...
            <p className="post__meta">
              ...
              <span><i className="fas fa-calendar-alt" style={{ color: "#FFC224", marginRight: 7, fontSize: 12 }} />Published <time dateTime={post.publishedAt?.toISOString().slice(0, 10)} itemProp="datePublished">{fmtDate(post.publishedAt)}</time></span>
              <span><i className="fas fa-sync-alt" style={{ color: "#FFC224", marginRight: 7, fontSize: 12 }} />Updated {fmtDate(post.updatedAt)}</span>
              <span><i className="fas fa-clock" style={{ color: "#FFC224", marginRight: 7, fontSize: 12 }} />{post.readMins} min read</span>
              <span><i className="fas fa-tag" style={{ color: "#FFC224", marginRight: 7, fontSize: 12 }} />{post.category}</span>
            </p>
          </div>
        </header>
        ...
        <div className="container post__layout">
          ...
          <div className="post__body" itemProp="articleBody">
            {takeaways.length ? (
              <div style={{ background: "#EFEFF2", borderRadius: 24, padding: "28px 30px", marginBottom: 38 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, fontFamily: P, fontWeight: 600, fontSize: 13, letterSpacing: 2, textTransform: "uppercase", color: "#5751E1", marginBottom: 18 }}><i className="fas fa-bolt" style={{ color: "#FFC224" }} />Key takeaways</div>
                <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 14 }}>
                  {takeaways.map((t, i) => <li key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start", fontSize: 16, lineHeight: 1.7, color: "#3d3c55" }}><span style={{ fontFamily: P, fontWeight: 700, fontSize: 11, color: "#fff", background: "#5751E1", width: 26, height: 26, borderRadius: 8, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 3 }}>{String(i + 1).padStart(2, "0")}</span><span>{t}</span></li>)}
                </ul>
              </div>
            ) : null}
            ...
            <div style={{ marginTop: 44, background: "linear-gradient(135deg,#161439 0%,#1C1A4A 60%,#5751E1 100%)", borderRadius: 26, padding: 32, position: "relative", overflow: "hidden" }}>
              <span style={{ position: "absolute", width: 220, height: 220, borderRadius: "50%", background: "#FFC224", filter: "blur(90px)", opacity: 0.25, right: -70, top: -70 }} />
            ...
            {dest.length ? (
              <section style={{ marginTop: 52, paddingTop: 36, borderTop: "1px solid #E6EAEF" }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, letterSpacing: -0.5, marginBottom: 6 }}>Explore destinations mentioned in this guide</h2>
                <p style={{ fontSize: 15, color: "#7F7E97", marginBottom: 18 }}>Country-wise costs, entry requirements, visa steps and stay-back rules.</p>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12 }}>
                  {dest.map((c) => (
                    <Link key={c.code} className="cv-ccard" href={`/destinations/${c.slug}`} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "1px solid #E6EAEF", borderRadius: 18, padding: "14px 16px", color: "#161439", transition: "all .35s", position: "relative", overflow: "hidden" }}>
                      <span className="cv-cfill" style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,#5751E1,#4640c9)", transform: "translateY(100%)", transition: "transform .45s cubic-bezier(.4,0,.2,1)" }} />
                      <img src={FLAG(c.code)} alt="" style={{ position: "relative", width: 28, height: 28, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
                      <span style={{ position: "relative", display: "flex", flexDirection: "column", minWidth: 0 }}><span className="cv-cname" style={{ fontFamily: P, fontWeight: 600, fontSize: 14.5, lineHeight: 1.2, transition: "color .3s" }}>Study in {c.name}</span><span className="cv-csub" style={{ fontSize: 12, color: "#7F7E97", transition: "color .3s" }}>{c.tag}</span></span>
                    </Link>
                  ))}
                </div>
              </section>
            ...
            {sources.length ? (
              <section style={{ marginTop: 48, paddingTop: 36, borderTop: "1px solid #E6EAEF" }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, letterSpacing: -0.5, marginBottom: 16 }}>Sources &amp; further reading</h2>
                <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
                  {sources.map((s, i) => <li key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", fontSize: 15, lineHeight: 1.7 }}><i className="fas fa-link" style={{ color: "#5751E1", fontSize: 12, marginTop: 6 }} /><a href={s.href} target="_blank" rel="noopener nofollow" style={{ color: "#5751E1" }}>{s.label}</a></li>)}
                </ul>
                <p style={{ marginTop: 16, fontSize: 13.5, lineHeight: 1.7, color: "#7F7E97" }}>Visa rules, fees and stay-back periods change frequently. Always confirm current requirements with the official source or with a Trinity counsellor before applying.</p>
              </section>
            ) : null}
            ...
            {adjacent.prev && adjacent.next ? (
              <nav style={{ marginTop: 44, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 12 }} aria-label="More articles">
                <Link className="cv-jcard" href={`/blog/${adjacent.prev.slug}`} style={jcard}>
                  <span className="cv-jfill" style={{ position: "absolute", inset: 0, background: "#EFEFF2", opacity: 0, transition: "opacity .4s" }} />
                  <span style={{ position: "relative", display: "block", fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#7F7E97", fontFamily: P, fontWeight: 600, marginBottom: 8 }}><i className="fas fa-arrow-left" style={{ color: "#FFC224", marginRight: 8 }} />Previous</span>
                ...
                <Link className="cv-jcard" href={`/blog/${adjacent.next.slug}`} style={{ ...jcard, textAlign: "right" }}>
                  <span className="cv-jfill" style={{ position: "absolute", inset: 0, background: "#EFEFF2", opacity: 0, transition: "opacity .4s" }} />
                  <span style={{ position: "relative", display: "block", fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#7F7E97", fontFamily: P, fontWeight: 600, marginBottom: 8 }}>Next<i className="fas fa-arrow-right" style={{ color: "#FFC224", marginLeft: 8 }} /></span>
            ...
            {post.tags.length ? (
              <div style={{ marginTop: 40, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
                <span style={{ fontFamily: P, fontWeight: 600, fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#7F7E97", marginRight: 6 }}>Topics</span>
                {post.tags.map((t) => <Link key={t} className="cv-jumper" href="/blog" style={{ fontFamily: P, fontWeight: 500, fontSize: 13.5, color: "#161439", background: "#EFEFF2", padding: "9px 16px", borderRadius: 50, transition: "all .3s" }}>#{t}</Link>)}
              </div>
            ) : null}
            ...
            <div className="post__author-box">
              ...
              <div>
                ...
                <span style={{ display: "block", fontSize: 13, color: "#5751E1", fontWeight: 500, margin: "2px 0 8px" }}>Reviewed by Trinity’s senior counselling desk · 30 years of student placements</span>
